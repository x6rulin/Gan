...
class _ConvLayer(torch.nn.Module):
    ...
    def __init__(self, in_channels, out_channels, kernel_size, stride, padding, bias=False, \
                 groups=1, transposed=False, output_padding=0, bn=True, activation='relu', **kwargs):
...
class _GLayer(torch.nn.Module):
    ...
    def __init__(self, in_channels, out_channels, kernel_size, stride, padding, transposed=True, output_padding=0, bn=True, activation='prelu', **kwargs):
        super(_GLayer, self).__init__()

        self.sub_module = _ConvLayer(in_channels, out_channels, kernel_size, stride, padding, \
                                     transposed=transposed, output_padding=output_padding, bn=bn, activation=activation, **kwargs)
...
class Discriminator(torch.nn.Module):
    ...
    def __init__(self):
        ...
        self.sub_module = torch.nn.Sequential(
            ...
            _DLayer(512, 1, 4, 1, 0, bn=False, activation='sigmoid'),
        )
...
class Generator(torch.nn.Module):
    ...
    def __init__(self, sample_num):
        ...
        self.sub_module = torch.nn.Sequential(
            ...
            _GLayer(64, 32, 5, 3, 1),
            _GLayer(32, 3, 3, 1, 1, transposed=False, bn=False, activation='tanh'),
        )
